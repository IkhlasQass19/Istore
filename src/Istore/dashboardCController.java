...
public class dashboardCController implements Initializable {
    ...
    @FXML
    private TableView<courseData> studentGrade_tableView1;
    ...
    @FXML
    private TableColumn<courseData, String> studentGrade_col_course1;


    @FXML
    private TableColumn<courseData, String> studentGrade_col_EmployeeNum1;
    @FXML
...
//        WE NEED TO FIX THE DELETE ON ADD STUDENT FORM
    ...
    public void StoreShowListData() throws SQLException {
        ...
        studentGrade_col_EmployeeNum1.setCellValueFactory(new PropertyValueFactory<>("id"));

        studentGrade_col_course1.setCellValueFactory(new PropertyValueFactory<>("email"));

        studentGrade_tableView1.setItems(StoreList);

    }


    public void studentGradesSelect1() throws SQLException {
      /*  courseData courseD = studentGrade_tableView1.getSelectionModel().getSelectedItem();
        int num = studentGrade_tableView1.getSelectionModel().getSelectedIndex();

        if ((num - 1) < -1) {
            return;
        }

        idstor=courseD.getId();
        studentGradesShowListData();*/
        courseData courseD = studentGrade_tableView1.getSelectionModel().getSelectedItem();
        int num = studentGrade_tableView1.getSelectionModel().getSelectedIndex();
        ...
        int userId = getData.id_user; // Assuming getData.id_user holds the user ID
        int storeId = courseD.getId(); // Assuming courseD.getId() holds the store ID

        try {
            connect = connectDb(); // Assumes connectDb() returns a valid Connection object

            // Prepare the SQL statement for inserting data into the access_log table
            String insertQuery = "INSERT INTO access_log (user_id, store_id, timestamp) VALUES (?, ?, CURRENT_TIMESTAMP)";
            ...
            // Optionally, you can show an alert or perform other actions to notify the user of the successful insertion
        } catch (SQLException e) {
            // Handle SQL exceptions (e.g., connection error, syntax error in the SQL statement)
            e.printStackTrace();
